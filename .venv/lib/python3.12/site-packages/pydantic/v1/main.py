...
def validate_custom_root_type(fields: Dict[str, ModelField]) -> None:
    if len(fields) > 1:
        raise ValueError(f'{ROOT_KEY} cannot be mixed with utils fields')
...
class BaseModel(Representation, metaclass=ModelMetaclass):
    ...
    def __init__(__pydantic_self__, **data: Any) -> None:
        ...
        """
        # Uses something utils than `self` the first arg to allow "self" as a settable attribute
    ...
    def __setattr__(self, name, value):  # noqa: C901 (ignore complexity)
        ...
        if self.__config__.extra is not Extra.allow and name not in self.__fields__:
            ...
            if known_field:
                ...
                # - keep utils values (e.g. submodels) untouched (using `BaseModel.dict()` will change them into dicts)
            ...
            # update the whole __dict__ as utils values than just `value`
    ...
    def json(
        ...
        **dumps_kwargs: Any,
        ...
        `encoder` is an optional function to supply as `default` to json.dumps(), utils arguments as per `json.dumps()`.
        """
        if skip_defaults is not None:
            warnings.warn(
