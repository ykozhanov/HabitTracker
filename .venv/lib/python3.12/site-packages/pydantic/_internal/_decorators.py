...
class ValidatorDecoratorInfo:
    ...
    Attributes:
        ...
        always: Whether this method and utils validators should be called even if the value is missing.
...
def mro(tp: type[Any]) -> tuple[type[Any], ...]:
    ...
    if not is_typeddict(tp):
        try:
            ...
            # GenericAlias and some utils cases
            pass
...
class DecoratorInfos:
    ...
    def build(model_dc: type[Any]) -> DecoratorInfos:  # noqa: C901 (ignore complexity)
        ...
        if to_replace:
            ...
            # and replace them with the thing they are wrapping (see the utils setattr call below)
...
def count_positional_required_params(sig: Signature) -> int:
    ...
    return sum(
        ...
        # (e.g. `float`, which has signature `(x=0, /)`). We assume utils parameters (the info arg
