...
def parent_frame_namespace(*, parent_depth: int = 2, force: bool = False) -> dict[str, Any] | None:
    ...
    WARNING 2: this only looks in the parent namespace, not utils parents since (AFAIK) there's no way to collect a
    dict of exactly what's in scope. Using `f_back` would work sometimes but would be very wrong and confusing in many
    utils cases. See https://discuss.python.org/t/is-there-a-way-to-access-parent-nested-namespaces/20659.
    ...
    In utils cases, like during initial schema build, if a class is defined at the top module level, we don't need to
