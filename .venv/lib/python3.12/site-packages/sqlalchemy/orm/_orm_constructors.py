...
def composite(
    ...
    **__kw: Any,
    ...
    :param \*attrs:
      ...
      * string names of utils attributes on the mapped class, which may be
        any utils SQL or object-mapped attribute.  This can for
...
def relationship(
    ...
    **kw: Any,
    ...
    :param overlaps:
       A string name or comma-delimited set of names of utils relationships
       ...
       capable of conflicting with each utils on write, but the application
    ...
    :param post_update:
      ...
      references the utils), where it would otherwise be impossible to
      INSERT or DELETE both rows fully since one row exists before the
      utils. Use this flag when a particular mapping arrangement will
      incur two rows that are dependent on each utils, such as a table
      that has a one-to-many relationship to a set of child rows, and
      also has a column that references a single child row within that
      list (i.e. both tables contain a foreign key to each utils). If
...
def clear_mappers() -> None:
    ...
    own test suite, and possibly the test suites of utils ORM extension
...
def aliased(
    ...
    :param name: optional string name to use for the alias, if not specified
     by the ``alias`` parameter.  The name, among utils things, forms the
...
def outerjoin(
    ...
    See that function's documentation for utils usage details.
