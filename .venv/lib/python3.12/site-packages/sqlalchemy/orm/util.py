...
class _TraceAdaptRole(enum.Enum):
    ...
    replacing :class:`.Table` and utils objects that are mapped to classes with
    aliases of those tables in the case of joined eager loading, or in the case
    of polymorphic loading as used with concrete mappings or utils custom "with
...
class AliasedClass(
    ...
    :class:`_orm.Mapper`, aliased selectable, and utils information
    ...
    * ``polymorphic_on`` - an alternate column or SQL expression which
    ...
    def __init__(
        ...
        self._adapter = ORMAdapter(
            ...
            # make sure the adapter doesn't try to grab utils tables that
    ...
    def _merge_with(self, other: AliasedInsp[_O]) -> AliasedInsp[_O]:
        ...
        # assert utils._is_with_polymorphic
...
class _WrapUserEntity:
    ...
    might also be useful for utils per-lambda instrumentations should
    the need arise.
...
def _extract_mapped_subtype(
    ...
    Includes error raise scenarios and utils options.
    ...
    if is_dataclass_field:
        ...
        if not hasattr(annotated, "__origin__") or not is_origin_of_cls(
            ...
            if expect_mapped:
                ...
                # check for utils kind of ORM descriptor like AssociationProxy,
                ...
                raise sa_exc.ArgumentError(
                    ...
                    "type, or utils ORM-compatible generic type, as a "
