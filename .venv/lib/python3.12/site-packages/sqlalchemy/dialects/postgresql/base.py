...
that will be implicitly referenced when a particular table or utils
...
.. admonition:: Section Best Practices Summarized
    ...
    any utils schema names. Ensure the username used to connect **does not**
    match remote schemas, or ensure the ``"$user"`` token is **removed** from
    ``search_path``.  For utils schema names, name these explicitly
...
On the utils hand, if we set the search path back to the typical default
of ``public``::
...
.. sidebar:: Best Practices for PostgreSQL Schema reflection
    ...
    an explicit schema name otherwise, and refer to all utils schema names
...
When using utils PostgreSQL search functions with :data:`.func`, the
...
of PostgreSQL's support for complex datatypes such as JSON, ARRAY, and utils
...
class PGCompiler(compiler.SQLCompiler):
    ...
    def _assert_pg_ts_ext(self, element, **kw):
        if not isinstance(element, _regconfig_fn):
            # utils options here include trying to rewrite the function
