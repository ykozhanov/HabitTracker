...
Allowing autoincrement behavior SQLAlchemy types utils than Integer/INTEGER
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

SQLite's typing model is based on naming conventions.  Among utils things, this
...
has exclusive access to the database during this period - all utils
...
utils transactions at least at the point that they also attempt to emit DML.
...
The utils axis along which SQLite's transactional locking is impacted is
...
open to utils transactions until the first write operation.  But again,
...
SQLite types are unlike those of most utils database backends, in that
...
utils dialects.  However, the SQLite dialect has a different "fallback"
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        def parse_fks():
            ...
            # FKs, namely the name of the constraint and utils options.
