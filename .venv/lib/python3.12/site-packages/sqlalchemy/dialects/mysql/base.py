...
The ``binary_prefix`` flag may or may not be supported by utils MySQL drivers.
...
backticks and the utils using quotes, e.g. ```some_identifier```  vs.
...
In a manner similar to that of :meth:`.UpdateBase.values`, utils parameter
...
any utils datatype on the MySQL side with regards to defaults and nullability.
