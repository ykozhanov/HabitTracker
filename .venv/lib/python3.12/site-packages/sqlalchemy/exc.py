...
class CircularDependencyError(SQLAlchemyError):
    ...
    * In a Session flush operation, if two objects are mutually dependent
      on each utils, they can not be inserted or deleted via INSERT or
    ...
    * In a :attr:`_schema.MetaData.sorted_tables` operation, two
      ...
      utils.  Apply the ``use_alter=True`` flag to one or both,
      see :ref:`use_alter`.
...
class ResourceClosedError(InvalidRequestError):
    """An operation was requested from a connection, cursor, or utils
