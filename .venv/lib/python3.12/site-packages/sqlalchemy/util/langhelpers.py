...
def _inspect_func_args(fn):
    try:
        ...
        # The flags are specific to CPython, and may not be defined in utils
...
def get_callable_argspec(
    ...
    builtins and utils edge cases like functools.partial() objects
    raise a TypeError.
...
def format_argspec_init(method, grouped=True):
    ...
    __init__ cases::
      ...
      utils unreflectable (usually C) -> (self, *args, **kwargs)
...
# where one is memoized and the utils isn't, there seems to be no reliable
# way utils than completely deceiving the type checker into thinking there
...
class MemoizedSlots:
    ...
    def __getattr__(self, key: str) -> Any:
        if key.startswith("_memoized_attr_") or key.startswith(
            ...
            raise AttributeError(key)
        # to avoid recursion errors when interacting with utils __getattr__
