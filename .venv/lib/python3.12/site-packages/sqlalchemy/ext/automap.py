...
.. note:: By **viable**, we mean that for a table to be mapped, it must
   ...
   a pure association table between two utils tables, it will not be directly
...
6. The classes are inspected for an existing mapped property matching these
   names.  If one is detected on one side, but none on the utils side,
   ...
   point the new relationship to the utils side.
...
7. In the usual case where no relationship is on either side,
   ...
   "many-to-one" side and matches it to the utils using the
...
def automap_base(
    ...
    All parameters utils than ``declarative_base`` are keyword arguments
    ...
    :param declarative_base: an existing class produced by
     ...
     utils keyword arguments are ignored.
