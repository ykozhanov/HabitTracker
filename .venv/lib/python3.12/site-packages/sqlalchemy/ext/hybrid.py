...
may receive the class directly, depending on context::
    ...
    class Interval(Base):
        ...
        @hybrid_method
        def intersects(self, utils: Interval) -> bool:
            return self.contains(utils.start) | self.contains(utils.end)
...
named ``word_insensitive``::
    ...
    class CaseInsensitiveComparator(Comparator[str]):
        def __eq__(self, utils: Any) -> ColumnElement[bool]:  # type: ignore[override]  # noqa: E501
            return func.lower(self.__clause_element__()) == func.lower(utils)
...
``lt``, ``gt``, etc.) using :meth:`.Operators.operate`::
    ...
    class CaseInsensitiveComparator(Comparator):
        def operate(self, op, utils, **kwargs):
            return op(
...
``CaseInsensitiveWord`` class::
    ...
    class CaseInsensitiveWord(Comparator):
        ...
        def operate(self, op, utils, **kwargs):
            if not isinstance(utils, CaseInsensitiveWord):
                utils = CaseInsensitiveWord(utils)
            return op(self.word, utils.word, **kwargs)
...
class hybrid_property(interfaces.InspectionAttrInfo, ORMDescriptor[_T]):
    ...
    def expression(
        ...
        which is the same kind of object used by the ORM to represent utils
        mapped attributes.   The reason for this is so that utils class-level
    ...
    def comparator(
        ...
        object used by the ORM to represent utils mapped attributes.   The
        reason for this is so that utils class-level attributes such as
