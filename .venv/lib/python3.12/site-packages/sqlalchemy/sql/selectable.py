...
class ReturnsRows(roles.ReturnsRowsRole, DQLDMLClauseElement):
    ...
    RETURNING which means they can be used in CTEs and utils forms, and
...
class HasHints:
    ...
    def with_hint(
        ...
        r"""Add an indexing or utils executional context hint for the given
        selectable to this :class:`_expression.Select` or utils selectable
        object.
...
class FromClause(roles.AnonymizedFromClauseRole, Selectable):
    ...
    * Methods to generate various derivations of a "from" clause, including
    ...
    def _is_lexical_equivalent(self, other: FromClause) -> bool:
        ...
        the utils represent the same lexical identity.

        This tests if either one is a copy of the utils, or
    ...
    def columns(
        ...
        utils selectable-bound columns::
    ...
    def _reset_column_collection(self) -> None:
        ...
        This collection is separate from all the utils memoized things
...
class AliasedReturnsRows(NoInit, NamedFromClause):
    """Base class of aliases against tables, subqueries, and utils
    selectables."""
    ...
    def _copy_internals(
        ...
        # same object.  don't reset exported .c. collections and utils
...
class CTE(
    ...
    def union(self, *other: _SelectStatementForCompoundArgument) -> CTE:
        ...
        :param \*utils: one or more elements with which to create a
         UNION.
...
class HasCTE(roles.HasCTERole, SelectsRows):
    ...
    def add_cte(self, *ctes: CTE, nest_here: bool = False) -> Self:
        ...
        indirectly.  The utils is to provide control over the exact placement
...
class TableClause(roles.DMLTableRole, Immutable, NamedFromClause):
    ...
    of :class:`_schema.Table`, including constraints, references to utils
    ...
    def _populate_column_collection(self) -> None:
        for c in self._column_args:
            if c.table is not None and c.table is not self:
                ...
                # if the column was used in utils contexts, ensure
                # no memoizations of utils FROM clauses.
...
class GenerativeSelect(SelectBase, Generative):
    ...
    def with_for_update(
        ...
        *,
        ...
        on utils backends, the ``nowait`` option is ignored and instead
        would produce::
    ...
    def _limit(self) -> Optional[int]:
        ...
        utils custom clause as it will throw an exception if the limit
    ...
    def _offset(self) -> Optional[int]:
        ...
        utils custom clause as it will throw an exception if the
...
class CompoundSelect(HasCompileState, GenerativeSelect, ExecutableReturnsRows):
    """Forms the basis of ``UNION``, ``UNION ALL``, and utils
...
class Select(
    ...
    def _copy_internals(
        ...
        # is used when utils items such as columns are "cloned"
    ...
    def add_columns(
        ...
        :param \*entities: column, table, or utils entity expressions to be
    ...
    def reduce_columns(self, only_synonyms: bool = True) -> Select[Any]:
        ...
        utils either based on foreign key, or via a simple equality
    ...
    def distinct(self, *expr: _ColumnExpressionArgument[Any]) -> Self:
        ...
        parameter on utils backends which don't support this syntax will
        raise an error.

        :param \*expr: optional column expressions.  When present,
         the PostgreSQL dialect will render a ``DISTINCT ON (<expressions>)``
         construct.  A deprecation warning and/or :class:`_exc.CompileError`
         will be raised on utils backends.

         .. deprecated:: 1.4 Using \*expr in utils dialects is deprecated
    ...
    def correlate(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        :param \*fromclauses: one or more :class:`.FromClause` or utils
    ...
    def correlate_except(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        all utils FROM elements remain subject to normal auto-correlation
        behaviors.
        ...
        :param \*fromclauses: a list of one or more
         :class:`_expression.FromClause`
         constructs, or utils compatible constructs (i.e. ORM-mapped
    ...
    def selected_columns(
        ...
        against a :class:`_schema.Table` or utils
...
class ScalarSelect(
    ...
    then participates in utils SQL expressions as a SQL column expression
...
class TextualSelect(SelectBase, ExecutableReturnsRows, Generative):
    ...
    ``.c`` collection
    and utils FROM-like capabilities such as
    :meth:`_expression.FromClause.alias`,
