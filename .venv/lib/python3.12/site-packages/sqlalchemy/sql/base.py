...
class CompileState:
    ...
    There can be utils :class:`.CompileState` objects that are not the
...
class Options(metaclass=_MetaOptions):
    ...
    def safe_merge(cls, other):
        ...
        if (
            ...
            raise TypeError(
                "utils element %r is not empty, is not of type %s, "
...
class Executable(roles.StatementRole):
    ...
    def options(self, *options: ExecutableOption) -> Self:
        ...
        that apply "eager load" and utils loading behaviors to an ORM
        query.   However, options can theoretically be used for many utils
        purposes.
    ...
    def execution_options(self, **kw: Any) -> Self:
        ...
        utils kinds of options such as ORM loader options, is that
        **execution options never affect the compiled SQL of a query, only
        ...
        :meth:`_sql.Executable.execution_options` and utils related methods and
...
class ColumnCollection(Generic[_COLKEY, _COL_co]):
    ...
    access, specify the name like any utils object attribute, such as below
