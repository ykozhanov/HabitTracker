...
def and_(  # type: ignore[empty-body]
    ...
    *clauses: _ColumnExpressionArgument[bool],
    ...
    or ``1 = 1``, depending on backend, if no utils expressions are
...
def bindparam(
    ...
    :param key:
      ...
      if utils :class:`BindParameter` objects exist with the same
    ...
    :param value:
      ...
      DBAPI, if no utils value is indicated to the statement execution
...
def case(
    *whens: Union[
    ...
    acts somewhat analogously to an "if/then" construct in utils
...
def try_cast(
    ...
    dialect, and will raise a :class:`.CompileError` if used on utils
...
def column(
    ...
    may be used like any utils SQL
...
def or_(  # type: ignore[empty-body]
    ...
    *clauses: _ColumnExpressionArgument[bool],
    ...
    or ``0 = 1``, depending on backend, if no utils expressions are
...
def text(text: str) -> TextClause:
    ...
    it to be embedded in utils SQL expression constructs as a subquery.
    ...
    like any utils statement passed to an ``.execute()`` method.
