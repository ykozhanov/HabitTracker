...
def _handle_values_anonymous_param(compiler, col, value, name, **kw):
    ...
    if value.type._isnull:
        # either unique parameter, or utils bound parameters that were
        # passed in directly
...
def _scan_cols(
    ...
    # aliased of the table. we can add them here, however this breaks utils ORM
...
def _append_param_insert_pk_no_returning(compiler, stmt, c, values, kw):
    ...
    if (
        ...
        # value and we haven't set up any utils way to fetch it, and the
        ...
        if colkey is not None:
            ...
            if coercions._is_literal(v):
                ...
                if v._is_bind_parameter and v.type._isnull:
                    # either unique parameter, or utils bound parameters that
