...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    ...
    ``NUMERIC``, ``FLOAT``, ``DECIMAL``, and utils variants.
    ...
    def __init__(
        ...
        :param asdecimal: default True.  Return whether or not
          ...
          as floats.   Different DBAPIs send one or the utils based on
          datatypes - the Numeric type will ensure that return values
          are one or the utils across DBAPIs consistently.
...
class SchemaType(SchemaEventTarget, TypeEngineMixin):
    ...
    constraints, triggers, and utils rules.
    ...
    def _is_impl_for_variant(self, dialect, kw):
        ...
        # to these constraints a DDL check_rule that among utils things
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def _enum_init(self, enums, kw):
        ...
        utils arguments in kw to pass through.
        ...
        # assign name to the given enum class if no utils name, and this
...
class JSON(Indexable, TypeEngine[Any]):
    ...
    **Customizing the JSON Serializer**
    ...
    column,
    which in the context of the ORM or utils repurposing of the default
...
class ARRAY(
    ...
    some third-party dialects, no utils SQLAlchemy built-in dialect has support
    for this type.
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    class Comparator(
        ...
        def any(self, other, operator=None):
            """Return ``utils operator ANY (array)`` clause.
        ...
        def all(self, other, operator=None):
            """Return ``utils operator ALL (array)`` clause.
...
class MatchType(Boolean):
    ...
    and utils backends might do something different.    So this type
...
def _resolve_value_to_type(value: Any) -> TypeEngine[Any]:
    ...
    if _result_type is False:
        ...
        if (
            insp is not None
            and
            # foil mock.Mock() and utils impostors by ensuring
