...
from backend.exceptions import AuthorizationError, HabitError, NotFoundError
...
class HabitController:
    ...
    async def get_habit_by_id(self, habit_id: int) -> Habit:
        habit_select = await self._session.execute(
    ...
    async def add(
        self, title: str, remind_time: time, description: Optional[str] = None
    ) -> Habit:
        new_habit = Habit(
            title=title,
    ...
    async def delete(self, habit_id: int) -> None:
        ...
        if habit is None:
            raise NotFoundError(
                "Привычка по id: {habit_id} не найдена.".format(habit_id=habit_id)
            )
    ...
    async def mark_complete_habit_by_id(self, habit_id: int) -> Habit:
        ...
        if (
            ...
            and datetime.now(tz=timezone.utc).date() == habit.last_time_check.date()
        ):
